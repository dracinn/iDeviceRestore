 ...
 * Recovery build, then libirecovery-compatible 0x41/0 initialization and (only if init succeeds)
 * personalized iBEC bulk upload. It never sends `go` or any restore-OS component.
 */
class PrearmedStage1IbecButton @JvmOverloads constructor(
        ...
        AlertDialog.Builder(activity)
            ...
            .setMessage(
                "This single confirmation authorizes two bounded boot-state changes so the app can act inside the short custom Stage-1 window without another tap. " +
                    "It will upload the validated personalized iBSS, wait for Recovery, require boot-stage=1 and build-version=$EXPECTED_STAGE1_BUILD, then issue the 0x41/0 iBEC upload initialization. " +
            ...
            .setPositiveButton("Pre-arm bounded test") { _, _ -> start() }
            .show()
    }
    ...
    private fun start() {
        ...
        log(activity, "prearmed Stage-1 iBEC test: explicit user confirmation received; boundary=iBSS-then-iBEC-no-go")
        ...
        worker.execute {
            ...
            try {
                ...
                connection.close()
                connection = null
                log(activity, "prearmed Stage-1 iBEC test: iBSS sent; waiting for fresh Recovery enumeration")

                val recovery = waitForExpectedStage1(activity, usb, ticket, transitionStarted)
                val identityKey = deviceIdentityKey(recovery)
                log(activity, "prearmed Stage-1 proof: boot-stage=1 build-version=$EXPECTED_STAGE1_BUILD; handing off immediately to PR #36 upload path")
                Stage1RecoveryProofStore.prove(identityKey, STAGE_1, EXPECTED_STAGE1_BUILD)
                ...
                require(liveStage == STAGE_1 && liveBuild == EXPECTED_STAGE1_BUILD) {
                    "Custom Stage-1 changed before iBEC init: boot-stage=$liveStage build-version=$liveBuild"
                }
    ...
    private fun waitForExpectedStage1(
                ...
                try {
                    ...
                    if (connection != null) {
                        ...
                        if (claimed != null) {
                            ...
                            if (seen != lastSeen) {
                                lastSeen = seen
                                log(activity, "prearmed Stage-1 candidate: boot-stage=${stage ?: "unknown"} build-version=${build ?: "unknown"}")
                            }
                            if (stage == STAGE_1 && build == EXPECTED_STAGE1_BUILD) return recovery
                        }
                    }
                } finally {
                    connection?.close()
        ...
        error("Timed out waiting for expected custom Stage-1 build $EXPECTED_STAGE1_BUILD")
    }
    ...
    companion object {
        ...
        private const val EXPECTED_STAGE1_BUILD = "mBoot-20457.1.29"
